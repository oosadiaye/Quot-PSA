...
from accounting.models import (
    Account, GLBalance, JournalHeader, JournalLine,
    FiscalPeriod, CostCenter
)
...
class TrialBalanceService:
    ...
    def generate_trial_balance_from_journals(
        ...
        for journal in journals:
            lines = JournalLine.objects.filter(journal_header=journal)
            for line in lines:
                ...
                accounts_data[account.id]['debit'] += line.debit_amount or Decimal('0')
                accounts_data[account.id]['credit'] += line.credit_amount or Decimal('0')
