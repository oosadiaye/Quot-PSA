...
class JournalSequenceService:
    ...
    # S1-05 — fiscal-year-scoped sequence key. Without the year in the key,
    # ``TransactionSequence`` reuses the same counter across all fiscal
    # years and silently ignores the prefix update after the first insert.
    @classmethod
    def _sequence_name(cls, fiscal_year: int | None = None) -> str:
        if fiscal_year:
            return f'journal:{fiscal_year}'
        return 'journal'
    ...
    def get_next_sequence(cls, fiscal_year: int = None) -> str:
        ...
        Scopes the ``TransactionSequence`` row by fiscal year so each year
        has its own independent counter. ``TransactionSequence.get_next``
        takes a row-level lock via ``select_for_update`` — safe under
        concurrent calls.
        """
        ...
        return TransactionSequence.get_next(
            cls._sequence_name(fiscal_year), prefix,
        )

    @classmethod
    def verify_sequence(cls, journal_id: int, fiscal_year: int = None) -> Tuple[bool, str]:
        """
        Verify that a journal's reference_number is consistent with the
        sequence counter.

        The previous implementation compared ``journal_id`` (a PK) to
        ``Max(id)+1`` (the next PK) — always False and meaningless. The
        correct check compares the numeric portion of the reference number
        to what the counter would produce at that point, and flags gaps.
        """
        ...
        try:
            ...
            return False, f"Journal {journal_id} not found"
        ref_num = journal.reference_number
        if not ref_num:
            return False, "Journal has no reference number"

        # Extract the numeric suffix (e.g. "JE2026-000042" → 42).
        import re
        m = re.search(r'(\d+)$', ref_num)
        if not m:
            return False, f"Reference number {ref_num!r} has no numeric suffix"
        seq_in_ref = int(m.group(1))

        missing = cls.find_missing_sequences(fiscal_year)
        if missing and seq_in_ref in missing:
            return False, (
                f"Reference {ref_num} corresponds to a missing sequence slot "
                f"(gap at {seq_in_ref})"
            )
        if missing:
            return False, f"Missing sequences detected elsewhere: {missing}"
        return True, f"Sequence is valid: {ref_num}"

    @classmethod
    def get_expected_sequence(cls, fiscal_year: int = None) -> int:
        """
        Get the expected next sequence number from the authoritative counter.

        Previously used ``Max('id')+1`` on ``JournalHeader`` which is
        unrelated to the sequence counter and produces nonsense. We now
        read directly from ``TransactionSequence``.
        """
        from accounting.models import TransactionSequence

        seq = TransactionSequence.objects.filter(
            name=cls._sequence_name(fiscal_year),
        ).first()
        if not seq:
            return 1
