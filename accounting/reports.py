...
class FinancialReportService:
    ...
    def get_account_balances(start_date: date, end_date: date, account_codes: List[str] = None) -> Dict:
        from accounting.models import JournalLine, Account, JournalHeader
        
        queryset = JournalLine.objects.filter(
            journal_header__date__gte=start_date,
            journal_header__date__lte=end_date,
            journal_header__status='Posted'
        ).select_related('account', 'journal_header')
        
        if account_codes:
            queryset = queryset.filter(account__code__startswith__in=account_codes)
...
class BudgetReportService:
    ...
    def generate_budget_vs_actual(
        ...
        from accounting.models import Fund, MDA
    ...
    def generate_cost_center_report(
        ...
        for cc in cost_centers:
            lines = JournalLineCostCenter.objects.filter(
                journal_line__journal_header__date__gte=start_date,
                journal_line__journal_header__date__lte=end_date,
                journal_line__journal_header__status='Posted',
                cost_center=cc
...
class IFRSReportService:
    ...
    def generate_ifrs_comparison_report(
        ...
        actuals = JournalLine.objects.filter(
            ...
            total_actual=Sum('debit_amount') - Sum('credit_amount')
        )
...
class InventoryReportService:
    ...
    def generate_stock_valuation(warehouse_id: int = None) -> Dict:
        from inventory.models import Item, ItemStock, Warehouse
...
class SalesReportService:
    @staticmethod
    def generate_sales_summary(start_date: date, end_date: date) -> Dict:
    ...
    @staticmethod
    def generate_customers_report() -> Dict:
        ...
        for customer in customers:
            ...
            total_sales = customer.sales_orders.aggregate(total=Sum('total_amount'))['total'] or Decimal('0')
...
class ProcurementReportService:
    ...
    def generate_purchase_summary(start_date: date, end_date: date) -> Dict:
        from procurement.models import PurchaseOrder, PurchaseInvoice
...
class ProductionReportService:
    @staticmethod
    def generate_production_summary(start_date: date, end_date: date) -> Dict:
        ...
        job_cards = JobCard.objects.filter(
            production_order__start_date__gte=start_date,
            production_order__start_date__lte=end_date
        )

        total_produced = orders.aggregate(total=Sum('quantity_produced'))['total'] or Decimal('0')
        total_labor_hours = job_cards.aggregate(total=Sum('time_actual'))['total'] or Decimal('0')
    ...
    @staticmethod
    def generate_material_consumption(start_date: date, end_date: date) -> Dict:
        from production.models import MaterialIssue
        
        issues = MaterialIssue.objects.filter(
            issue_date__gte=start_date,
            issue_date__lte=end_date
        ).select_related('bom_line__component')
        
        by_item = issues.values('bom_line__component__item_code', 'bom_line__component__item_name').annotate(
            total_quantity=Sum('quantity_issued')
        )
        
        return {
            'report_type': 'Material Consumption',
            'period': {'start': start_date, 'end': end_date},
            'materials': list(by_item),
            'total_issues': issues.count()
        }

    @staticmethod
    def generate_production_cost_report(start_date: date, end_date: date) -> Dict:
        from production.models import ProductionOrder, MaterialIssue, JobCard, BOMLine, BillOfMaterials
        ...
        for order in completed_orders:
            ...
            for issue in material_issues:
                component = issue.bom_line.component
                qty_issued = issue.quantity_issued
                unit_cost = component.standard_cost or Decimal('0')
                raw_material_cost += qty_issued * unit_cost
            
            labor_cost = JobCard.objects.filter(
                production_order=order
            ).aggregate(total=Sum('labor_cost'))['total'] or Decimal('0')
            
            total_production_cost = raw_material_cost + labor_cost
            
            quantity_produced = order.quantity_produced or Decimal('1')
            unit_production_cost = total_production_cost / quantity_produced
            
            # Use BOM inventory_item FK for reliable price lookup;
            # fall back to string match on item_name for unlinked BOMs
            sales_price = bom.standard_cost or Decimal('0')
            if bom.inventory_item_id:
                # Prefer the inventory item's selling price
                inv_price = bom.inventory_item.selling_price
                if inv_price and inv_price > 0:
                    sales_price = inv_price
            else:
                # Legacy fallback: string match against SalesOrderLine descriptions
                avg_sales = SalesOrderLine.objects.filter(
                    item_description__iexact=bom.item_name
                ).aggregate(avg_price=Avg('unit_price'))['avg_price']
                if avg_sales:
                    sales_price = avg_sales
            
            total_revenue = quantity_produced * sales_price
        ...
        total_produced_qty = sum(p['quantity_produced'] for p in product_costs)
        total_raw_material_cost = sum(p['raw_material_cost'] for p in product_costs)
        total_labor_cost = sum(p['labor_cost'] for p in product_costs)
        total_cost = sum(p['total_production_cost'] for p in product_costs)
        total_revenue = sum(p['total_revenue'] for p in product_costs)
        total_profit = sum(p['profit'] for p in product_costs)
        
        avg_profit_margin = (total_profit / total_revenue * 100) if total_revenue else Decimal('0')
        
        return {
            'report_type': 'Production Cost & Profitability Analysis',
            'period': {'start': start_date, 'end': end_date},
            'products': product_costs,
            'summary': {
                'total_orders': len(product_costs),
                'total_quantity_produced': total_produced_qty,
    ...
    @staticmethod
    def generate_product_profitability(start_date: date, end_date: date) -> Dict:
        from production.models import ProductionOrder, MaterialIssue, JobCard
        from sales.models import SalesOrderLine
        ...
        for order in completed_orders:
            ...
            raw_material_cost = Decimal('0')
            for issue in MaterialIssue.objects.filter(production_order=order).select_related('bom_line__component'):
                component = issue.bom_line.component
                raw_material_cost += issue.quantity_issued * (component.standard_cost or Decimal('0'))
            
            labor_cost = JobCard.objects.filter(
                production_order=order
            ).aggregate(total=Sum('labor_cost'))['total'] or Decimal('0')
            
            sales_prices = SalesOrderLine.objects.filter(
                item_description__icontains=bom.item_name
            ).values_list('unit_price', flat=True)
            
            products_data[product_key]['orders_count'] += 1
            products_data[product_key]['total_quantity'] += order.quantity_produced or Decimal('0')
            products_data[product_key]['total_material_cost'] += raw_material_cost
            products_data[product_key]['total_labor_cost'] += labor_cost
            if sales_prices:
                products_data[product_key]['selling_prices'].extend(list(sales_prices))
        
        profitability = []
        for product_key, data in products_data.items():
            total_cost = data['total_material_cost'] + data['total_labor_cost']
            avg_selling_price = sum(data['selling_prices']) / len(data['selling_prices']) if data['selling_prices'] else Decimal('0')
            unit_cost = total_cost / data['total_quantity'] if data['total_quantity'] else Decimal('0')
            unit_profit = avg_selling_price - unit_cost
            profit_margin = (unit_profit / avg_selling_price * 100) if avg_selling_price else Decimal('0')
            
            profitability.append({
                'product_code': data['product_code'],
                'product_name': data['product_name'],
                'orders_count': data['orders_count'],
                'total_quantity_produced': data['total_quantity'],
                ...
                'profitability_status': 'High' if profit_margin > 20 else ('Medium' if profit_margin > 10 else 'Low')
            })
        
        profitability.sort(key=lambda x: x['profit_margin_percent'], reverse=True)
        
        return {
            'report_type': 'Product Profitability Analysis',
            'period': {'start': start_date, 'end': end_date},
            'products': profitability,
            'summary': {
                'total_products': len(profitability),
                'high_profit_count': len([p for p in profitability if p['profitability_status'] == 'High']),
                'medium_profit_count': len([p for p in profitability if p['profitability_status'] == 'Medium']),
                'low_profit_count': len([p for p in profitability if p['profitability_status'] == 'Low'])
            }
        }
...
class TaxReportService:
    ...
    def generate_withholding_tax_report(period_start: date, period_end: date) -> Dict:
        ...
        from accounting.models import WithholdingTax, JournalLine, JournalHeader
