...
Ported from feat/budget-line-code, which had these before this branch
did. Kept as its own module rather than folded into
``tests/test_budget_code.py``: that file covers how the field behaves in
the database — not rewritten on save, not unique — while this one covers
the column contract, which needs no database at all and runs in
milliseconds.
"""
