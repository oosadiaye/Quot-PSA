/**
 * IPSAS — Geographic Distribution Report — Quot PSE
 * Route: /accounting/ipsas/geographic-distribution
 */
...
export default function GeographicDistributionReport() {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['ipsas-geographic-distribution', fyId],
        queryFn: async () => {
            return (await apiClient.get('/accounting/ipsas/geographic-distribution/', { params: { fiscal_year: fyId } })).data;
        },
        enabled: !!fyId,
        retry: false,
    });
    ...
    const filteredRows = useMemo(() => {
        ...
        return rows.filter((r: any) =>
            (r.geo__code ?? '').toLowerCase().includes(needle) ||
            (r.geo__name ?? '').toLowerCase().includes(needle),
        );
    }, [data, filter]);
        ...
        <div style={{ background: 'var(--color-surface-hover)', minHeight: '100vh' }}>
            ...
            <main style={{ marginLeft: '260px', padding: '32px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                    <div>
                        <h1 style={{ fontSize: '24px', fontWeight: 800, color: 'var(--color-text)', margin: 0 }}>Geographic Distribution Performance Report</h1>
                        <p style={{ color: 'var(--color-text-muted)', fontSize: '14px', margin: '4px 0 0' }}>Budget vs Actual Expenditure by Geographic Location — Utilization Analysis</p>
                        {data?.budget_source === 'pro_rata' && (
                            <div style={{
                                ...
                                ⚠ Pro-rata estimate — no geo-dimensioned budget on file
                            </div>
                        )}
                        {data?.budget_source === 'unavailable' && (
                            <div style={{
                                marginTop: 8, display: 'inline-flex', alignItems: 'center', gap: 6,
                                padding: '4px 10px', borderRadius: 999,
                    ...
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        ...
                        <ExportExcelButton
                            endpoint="/accounting/ipsas/geographic-distribution/"
                            params={{ fiscal_year: fyId || undefined }}
                            filename={`geographic-distribution-${fyId}.xlsx`}
                        />
                    </div>
                </div>
                ...
                <FilterBar
                    value={filter}
                    onChange={setFilter}
                    placeholder="Filter by geographic code or name…"
                ...
                {isLoading ? (
                    ...
                    <ReportError error={error} endpoint="/accounting/ipsas/geographic-distribution/" />
                ) : data ? (
                    <div style={{ background: 'var(--color-surface)', borderRadius: '12px', border: '1px solid var(--color-border)', overflow: 'hidden' }}>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                                <thead>
                                    <tr style={{ background: 'var(--color-surface-hover)', borderBottom: '2px solid var(--color-border)' }}>
                                        {['Geo Code', 'Location', 'Budget', 'Actual', 'Variance', '% Utilization', '% of Total'].map((h, i) => (
                                            <th key={h} style={{ padding: '12px 14px', textAlign: i < 2 ? 'left' : 'right', fontSize: '11px', fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredRows.map((row: any, idx: number) => {
                                        ...
                                        return (
                                            <tr key={idx} style={{ borderBottom: '1px solid var(--color-border-light)' }}>
                                                <td style={{ padding: '10px 14px', fontSize: '13px', fontFamily: 'monospace', fontWeight: 500 }}>{row.geo__code}</td>
                                                <td style={{ padding: '10px 14px', fontSize: '13px' }}>{row.geo__name}</td>
                                                ...
                                                <td style={{ padding: '10px 14px', fontSize: '13px', textAlign: 'right', fontWeight: 700, color: 'var(--color-text-muted)' }}>{parseFloat(row.pct_of_total ?? '0').toFixed(1)}%</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
                        ...
                        {(!data.rows || data.rows.length === 0) && (
                            <div style={{ textAlign: 'center', padding: '40px', color: 'var(--color-text-subtle)' }}>No geographic distribution data for this fiscal year.</div>
                        )}
                    </div>
                ) : null}
            </main>
